function [x, f1] = CSL1NlCg_4DRadial(x0,param)
...
while(1)
    ...
    if (k > param.nite) || (norm(dx(:)) < gradToll)
        break;
    end
...
if param.L1Weight
    L1Obj=0;
    for jj=1:size(x,4)
        w = wavedec3(x(:,:,:,jj)+t*dx(:,:,:,jj),2,'db2');
        for ii=1:1:15 %use ii=1:1:1 for w.dec{1} only ??
            temp=w.dec{ii}; 
